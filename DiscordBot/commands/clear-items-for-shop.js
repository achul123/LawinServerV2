...
module.exports = {
  ...
  execute: async (interaction) => {
    ...
    const CatalogConfig = JSON.parse(
      fs.readFileSync("./Config/catalog_config.json").toString()
    );
    ...
    if (StatChanged) {
      ...
      if (global.Clients.some((i) => i.accountId == targetUser.accountId))
        global.giftReceived[targetUser.accountId] = true;
      ...
      return interaction.editReply({
        content:
          "Successfully cleared items in your profile that are from the item shop.",
      });
    }

    interaction.editReply({
      content: `You do not own any items that are from the item shop.`,
      ephemeral: true,
    });
  },
};
