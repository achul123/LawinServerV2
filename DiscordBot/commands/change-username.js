...
module.exports = {
  ...
  execute: async (interaction) => {
    ...
    if (username.length >= 25)
      return interaction.editReply({
        content: "Your username must be less than 25 characters long.",
        ephemeral: true,
      });
    if (username.length < 3)
      return interaction.editReply({
        content: "Your username must be atleast 3 characters long.",
        ephemeral: true,
      });
    ...
    try {
      await user.updateOne({
        $set: { username: username, username_lower: username.toLowerCase() },
      });
    } catch {
      return interaction.editReply({
        content: "This username is already in use!",
        ephemeral: true,
      });
    }

    interaction.editReply({
      content: `Successfully changed your username to ${username}`,
      ephemeral: true,
    });
  },
};
