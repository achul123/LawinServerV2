    ...
    switch (msg.root.name) {
      ...
      case "auth":
        ...
        if (
          ...
          Authenticated = true;
          log.xmpp(
            `An xmpp client with the displayName ${displayName} has logged in.`
          );
...
function RemoveClient(ws, joinedMUCs) {
  ...
  if (partyId && typeof partyId == "string") {
    global.Clients.forEach((ClientData) => {
      ...
      ClientData.client.send(
        XMLBuilder.create("message")
          .attribute("id", functions.MakeID().replace(/-/gi, "").toUpperCase())
  ...
  log.xmpp(
    `An xmpp client with the displayName ${client.displayName} has logged out.`
  );
}
